...
//
// Gets a generic queue and searchs if a specific Thread is in it.
//
static
FORCEINLINE
...
static
FORCEINLINE
BOOL IsHandleValid(HANDLE thread){
	// Check if the thread is in the ReadyQueue List
	if (IsThreadInQueue(&ReadyQueue, thread)){
		return TRUE;
	}
	//Check if the thread is in the DeactivatedQueue List
	else if (IsThreadInQueue(&DeactivatedQueue, thread)){
		return TRUE;
	}
	return FALSE;
}
...
//
// Acrescente à biblioteca UThread a função int UtJoin(HANDLE thread) que não retorna 
// enquanto não terminar a thread correspondente ao handle passado como argumento. A função 
// retorna -1 se o handle passado não corresponder a uma thread em actividade. Em caso de 
// sucesso a função retorna 0. Considere uma thread em actividade, qualquer thread que ainda não 
...
DWORD UtJoin(HANDLE thread){
	...
	// Register the thread as the thread that I am waiting for to finish, so I can proceed later.
	RunningThread->WaitingThread = thread;
...
VOID UtInit() {
	...
	InitializeListHead(&DeactivatedQueue);
}
